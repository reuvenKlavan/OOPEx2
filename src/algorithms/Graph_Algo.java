...
public class Graph_Algo implements graph_algorithms{
	...
	public double shortestPathDist(int src, int dest) {
		...
		if(reset == true) {//check if we need to reset the graph for using this algorithem
			reset();
			reset = false;
		}
		if(src==dest) {// stop recursive case
		...
		if(source != null) {
			source.setTag(2);//to make sure not returning on the same vertex in the search for not to get infinity loop
			Collection<edge_data> srcEdge =  algo.getE(src);
			for(edge_data e: srcEdge){//passing on the edges for looking to dest
				node_data next = algo.getNode(e.getDest());
				if(next.getTag() == 0) {	
					if(source.getWeight() == Double.MAX_VALUE)//if we never got to you we will initiate you and give you a weight afterward 
						source.setWeight(0);
					
					double test = e.getWeight()+source.getWeight();//check if we find a new minimal distance
					
					if(e.getWeight()+source.getWeight()< next.getWeight())//checking if we need to give the vertex we going to pass a new weight  
						next.setWeight(e.getWeight()+source.getWeight());
				
					double tmp = shortestPathDist(e.getDest(), dest);// a recursive call
					if(tmp <= output && tmp>=test) {
						output = tmp;
						String n = ((Integer)source.getKey()).toString();
						next.setInfo(n);//set the vertex we got from to this vertex. this is helping for the implementation of shortestPath method
						}	
					}
				}

			source.setTag(0);//changing the vertex back to white so if for next checks
